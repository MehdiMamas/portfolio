...
var app = new Vue({
    ...
    methods: {
        linksWhoVoted: (e) => {
            ...
            window.open(chrome.extension.getURL("/lib/index1.html") + `?dataId=${dataId}&dataName=${name}`)
        }
    },
})
document.querySelector("#showLinks").addEventListener("click", function (e) {
    window.open(chrome.extension.getURL("/lib/index2.html") + `?dataId=${dataId}&dataName=${name}`)
})
